...
bool four_pcs_registration_t<DataType>::align_impl(result_type& result)
{
  ...
  LOG_INFO_S << "4PCS: 开始配准 / Starting registration"
  ...
  LOG_INFO_S << "4PCS: 采样完成 / Sampling complete"
  ...
  auto source_bases = extract_coplanar_bases(m_source_samples, this->get_source_cloud(), m_num_bases);
  if (source_bases.empty()) {
    LOG_ERROR_S << "4PCS: 无法提取有效的4点基 / Failed to extract valid 4-point bases";
    return false;
  }

  LOG_INFO_S << "4PCS: 提取了 " << source_bases.size() << " 个4点基 / 4-point bases";
  ...
  for (std::size_t base_idx = 0; base_idx < source_bases.size(); ++base_idx) {
    ...
    auto target_bases = find_congruent_sets(source_base, m_target_samples, this->get_target_cloud());
    ...
    if (best_candidate.num_inliers >= m_source_samples.size() * m_overlap * 0.9) {
      LOG_INFO_S << "4PCS: 早停，找到足够好的匹配 / Early stopping, found good match";
  ...
  LOG_INFO_S << "4PCS: 完成 " << result.num_iterations << " 个基的匹配，共 "
  ...
  LOG_INFO_S << "4PCS: 配准完成 / Registration complete"
...
bool four_pcs_registration_t<DataType>::validate_input_impl() const
{
  ...
  if (source_cloud->size() < 4 || target_cloud->size() < 4) {
    LOG_ERROR_S << "4PCS: 点云太小，至少需要4个点 / Point clouds too small, need at least 4 points";
    return false;
  }
...
template<typename DataType>
void four_pcs_registration_t<DataType>::set_source_impl(const point_cloud_ptr& source)
{
...
template<typename DataType>
void four_pcs_registration_t<DataType>::set_target_impl(const point_cloud_ptr& target)
{
...
template<typename DataType>
void four_pcs_registration_t<DataType>::sample_points(const point_cloud_ptr& cloud,
                                                      ...
                                                      std::vector<std::size_t>& indices)
{
  ...
  // 如果请求的样本数大于等于点云大小，使用所有点 / If requested samples >= cloud size, use all points
    ...
    std::size_t num_bases) const
{
  ...
  const DataType coplanar_tolerance = m_delta * 2;  // 共面容差 / Coplanarity tolerance
  std::size_t attempts = 0;
  const std::size_t max_attempts = num_bases * 100;  // 最大尝试次数 / Max attempts
  ...
  while (bases.size() < num_bases && attempts < max_attempts) {
    ...
    for (std::size_t i = 0; i < 4; ++i) {
      for (std::size_t j = i + 1; j < 4; ++j) {
        ...
        if (dist < m_delta * 10) {  // 点间最小距离 / Min distance between points
          valid = false;
          break;
        }
      }
...
template<typename DataType>
bool four_pcs_registration_t<DataType>::are_coplanar(const std::array<vector3_t, 4>& points,
...
template<typename DataType>
void four_pcs_registration_t<DataType>::compute_invariants(base_4pcs_t& base) const
{
  // 4PCS使用两个交点的比率作为仿射不变量 / 4PCS uses ratios of intersection points as affine invariants
  // 对角线AC和BD的交点 / Intersection of diagonals AC and BD
  ...
  // 计算交点（使用参数方程） / Compute intersection (using parametric equations)
  // P0 + s * AC_dir = P1 + t * BD_dir
  
  // 构建线性系统 / Build linear system
  matrix3_t A;
  A.col(0) = ac_dir;
  A.col(1) = -bd_dir;
  A.col(2) = base.normal;  // 添加法向量确保系统非奇异 / Add normal to ensure non-singular
  ...
  // 计算不变量（交点在对角线上的位置比率） / Compute invariants (position ratios on diagonals)
    ...
    const point_cloud_ptr& target_cloud) const
{
  ...
  const DataType distance_tolerance = m_delta * 2;  // 距离容差 / Distance tolerance
  ...
  for (std::size_t i = 0; i < 4; ++i) {
    for (std::size_t j = i + 1; j < 4; ++j) {
      source_distances[edge_idx++] = (source_base.points[i] - source_base.points[j]).norm();
    }
  }
  
  // 寻找所有可能的匹配 / Find all possible matches
  // 这里简化实现，实际4PCS使用更复杂的搜索策略 / Simplified implementation, actual 4PCS uses more complex search
  ...
  for (std::size_t try_idx = 0; try_idx < max_tries; ++try_idx) {
    ...
    for (std::size_t i = 0; i < 4; ++i) {
      for (std::size_t j = i + 1; j < 4; ++j) {
        target_distances[edge_idx++] = (target_base.points[i] - target_base.points[j]).norm();
      }
    }
    
    // 检查边长是否匹配（允许不同的点对应关系） / Check if edge lengths match (allow different correspondences)
    bool distances_match = true;
    for (std::size_t i = 0; i < 6; ++i) {
      bool found_match = false;
      for (std::size_t j = 0; j < 6; ++j) {
        if (std::abs(source_distances[i] - target_distances[j]) < distance_tolerance) {
          found_match = true;
          break;
        }
      }
...
four_pcs_registration_t<DataType>::estimate_transformation(const base_4pcs_t& source_base,
                                                          const base_4pcs_t& target_base) const
{
  transformation_t transform;
  transform.setIdentity();
  
  // 使用SVD估计变换（类似RANSAC） / Estimate transformation using SVD (similar to RANSAC)
...
template<typename DataType>
void four_pcs_registration_t<DataType>::refine_candidate(candidate_t& candidate) const
{
  // 可以在这里添加ICP或其他精炼方法 / Can add ICP or other refinement methods here
  // 目前保持简单，只重新计算LCP / Keep it simple for now, just recompute LCP
