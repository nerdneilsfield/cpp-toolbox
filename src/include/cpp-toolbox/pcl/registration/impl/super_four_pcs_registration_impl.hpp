    ...
    DataType distance, DataType epsilon, std::size_t max_pairs) const
{
  ...
  for (const auto& [center_key, center_cell] : m_grid) {
    if (center_cell.point_indices.empty())
      continue;
        ...
        std::size_t start_j = (center_key == neighbor_key)
            ? std::distance(center_cell.point_indices.begin(),
                            std::find(center_cell.point_indices.begin(),
                                      center_cell.point_indices.end(),
                                      i))
                + 1
            : 0;

        for (std::size_t j_idx = start_j;
             j_idx < neighbor_cell.point_indices.size();
             ++j_idx)
        {
...
template<typename DataType>
std::string
super_four_pcs_registration_t<DataType>::smart_index_t::get_statistics() const
{
  std::stringstream ss;
  ss << "Smart Index Statistics:\n";
  ss << "  Grid cells: " << m_grid.size() << "\n";
  ss << "  Cell size: " << m_cell_size << "\n";
  ss << "  Bounds: [" << m_min_bound.transpose() << "] to ["
     << m_max_bound.transpose() << "]\n";
  ...
  for (const auto& [key, cell] : m_grid) {
    ...
    max_points_per_cell =
        std::max(max_points_per_cell, cell.point_indices.size());
  }

  ss << "  Total indexed points: " << total_points << "\n";
  ss << "  Max points per cell: " << max_points_per_cell << "\n";
  ss << "  Avg points per cell: "
     << static_cast<double>(total_points) / m_grid.size() << "\n";
...
super_four_pcs_registration_t<DataType>::smart_index_t::compute_grid_key(
...
{
  // 如果禁用智能索引，使用基类实现 / If smart indexing disabled, use base class
  // implementation
  if (!m_use_smart_indexing) {
    LOG_DEBUG_S << "Super4PCS: 智能索引已禁用，使用标准4PCS / Smart indexing "
                   "disabled, using standard 4PCS";
  ...
  LOG_DEBUG_S << "Super4PCS: 开始配准（智能索引模式） / Starting registration "
                 "(smart indexing mode)"
              << ", 源点云大小 / source size: "
  ...
  if (grid_res <= 0) {
    grid_res = compute_adaptive_grid_resolution(this->get_source_cloud(),
                                                source_samples);
    LOG_DEBUG_S
        << "Super4PCS: 自动计算网格分辨率 / Auto-computed grid resolution: "
        << grid_res;
  }
  ...
  LOG_DEBUG_S << "Super4PCS: 索引构建完成 / Index building complete";
  LOG_DEBUG_S << "源索引 / Source index: " << m_source_index->get_statistics();
  LOG_DEBUG_S << "目标索引 / Target index: "
              << m_target_index->get_statistics();

  // 提取源点云的共面4点基（使用优化方法） / Extract coplanar bases (using
  // optimized method)
  ...
  for (std::size_t i = 0;
       i < num_distance_samples && i < source_samples.size() / 2;
       ++i)
  {
  ...
  for (const DataType& distance : base_distances) {
    ...
    auto source_pairs = extract_pairs_smart(source_samples,
                                            this->get_source_cloud(),
                                            distance,
                                            m_pair_distance_epsilon);

    // 从点对构建4点基 / Build 4-point bases from pairs
    for (std::size_t i = 0;
         i < source_pairs.size() && source_bases.size() < this->get_num_bases();
         ++i)
    {
      for (std::size_t j = i + 1; j < source_pairs.size()
           && source_bases.size() < this->get_num_bases();
           ++j)
      {
        if (verify_pair_compatibility(source_pairs[i], source_pairs[j])) {
          auto base_opt = build_base_from_pairs(
              source_pairs[i], source_pairs[j], this->get_source_cloud());
  ...
  if (source_bases.empty()) {
    LOG_ERROR_S << "Super4PCS: 无法提取有效的4点基 / Failed to extract valid "
                   "4-point bases";
    return false;
  }

  LOG_DEBUG_S << "Super4PCS: 提取了 " << source_bases.size()
              << " 个4点基 / 4-point bases";
  ...
  // 对每个源基寻找匹配（使用优化方法） / Find matches for each source base
  ...
  for (std::size_t base_idx = 0; base_idx < source_bases.size(); ++base_idx) {
    ...
    for (const auto& target_base : target_bases) {
      ...
      candidate.transform =
          this->estimate_transformation(source_base, target_base);
      ...
      candidate.lcp_score =
          this->compute_lcp_score(candidate.transform, inliers);
    ...
    if (best_candidate.num_inliers
        >= source_samples.size() * this->get_overlap() * 0.9)
    {
      LOG_DEBUG_S << "Super4PCS: 早停，找到足够好的匹配 / Early stopping, "
                     "found good match";
  ...
  LOG_DEBUG_S << "Super4PCS: 完成 " << result.num_iterations
              << " 个基的匹配，共 " << total_candidates
  ...
  LOG_DEBUG_S << "Super4PCS: 配准完成 / Registration complete"
    ...
    const base_4pcs_t& source_base) const
{
  ...
  DataType diag1_len =
      (source_base.points[2] - source_base.points[0]).norm();  // AC
  DataType diag2_len =
      (source_base.points[3] - source_base.points[1]).norm();  // BD
...
super_four_pcs_registration_t<DataType>::build_base_from_pairs(
    const point_pair_t& pair1,
    const point_pair_t& pair2,
...
template<typename DataType>
DataType
super_four_pcs_registration_t<DataType>::compute_adaptive_grid_resolution(
    const point_cloud_ptr& cloud, const std::vector<std::size_t>& indices) const
{
  ...
  const std::size_t sample_size =
      std::min(static_cast<std::size_t>(100), indices.size());
  ...
  for (std::size_t i = 0; i < sample_size; ++i) {
    ...
    for (std::size_t j = 0;
         j < std::min(static_cast<std::size_t>(10), indices.size());
         ++j)
    {
      if (indices[j] == idx)
        continue;
