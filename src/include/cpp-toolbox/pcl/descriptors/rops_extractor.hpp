...
struct rops_signature_t : public base_signature_t<rops_signature_t<DataType>>
{
  ...
  DataType distance(const rops_signature_t& other) const
  {
    DataType dist = 0;
  ...
  bool operator==(const rops_signature_t& other) const
  {
...
class rops_extractor_t
    : public base_descriptor_extractor_t<rops_extractor_t<DataType, KNN>,
                                        DataType, rops_signature_t<DataType>>
{
public:
  ...
  void compute_impl(const toolbox::types::point_cloud_t<DataType>& cloud,
                    const std::vector<std::size_t>& keypoint_indices,
                    std::unique_ptr<std::vector<rops_signature_t<DataType>>> descriptors) const
  {
    descriptors->clear();
                    ...
                    rops_signature_t<DataType>& descriptor) const
  {
    ...
    for (std::size_t r = 0; r < num_rotations_; ++r)
    {
      ...
      for (const auto& pt : rotated_points)
      {
        ...
        std::size_t x_idx = static_cast<std::size_t>(
          normalized(0) * num_partitions_x_);
        std::size_t y_idx = static_cast<std::size_t>(
          normalized(1) * num_partitions_y_);
        std::size_t z_idx = static_cast<std::size_t>(
          normalized(2) * num_partitions_z_);
