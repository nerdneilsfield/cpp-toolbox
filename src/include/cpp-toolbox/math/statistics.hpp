...
/**
 * @brief 检查容器是否为空并抛出异常/Check if the container is empty and throw an exception
 * 
 ...
 * 
 * @note 若容器为空则抛出 std::invalid_argument 异常/Throws std::invalid_argument if container is empty
 * 
 * @code
 * std::vector<int> v;
...
/**
 * @brief 计算容器中元素的平均值/Compute the mean (average) of elements in a container
 * 
 * @tparam TContainer 容器类型，元素需为算术类型/Container type, elements must be arithmetic
 * @param data 输入容器/Input container
...
double mean(const TContainer& data)
{
  ...
  for (const auto& val : data) {
    sum += static_cast<double>(val);  // 使用 double 进行累加保证精度/Use double for accumulation to ensure precision
  }
 ...
 * 
 * @tparam TContainer 容器类型，元素需为算术类型且可比较/Container type, elements must be arithmetic and comparable
 * @param data 输入容器/Input container
...
double median(const TContainer& data)
{
  ...
  std::vector<ValueType> sorted_data(data.begin(),
...
/**
 * @brief 计算容器中元素的众数（可能有多个）/Compute the mode(s) of elements in a container (may be multiple)
 * 
 * @tparam TContainer 容器类型，元素需可比较/Container type, elements must be comparable
 * @param data 输入容器/Input container
 * @return std::vector<typename TContainer::value_type> 众数集合/Vector of mode(s)
 * 
...
std::vector<typename TContainer::value_type> mode(const TContainer& data)
{
  ...
  if (max_freq > 0) {  // 确保至少有一个元素/Ensure at least one element
    ...
    // 为了结果的一致性，对众数进行排序 (如果它们本身是可比较的)/Sort modes for consistency (if comparable)
    // 如果 ValueType 不支持排序，但作为 map key 必须支持，这里 sort 应该也没问题/If ValueType is not sortable, but as map key it must be, so sort is fine here
...
/**
 * @brief 计算方差（默认样本方差 N-1）/Compute the variance (default is sample variance N-1)
 * 
 * @tparam TContainer 容器类型，元素需为算术类型/Container type, elements must be arithmetic
 * @param data 输入容器/Input container
 * @param sample_variance 是否计算样本方差（true）或总体方差（false）/Whether to compute sample variance (true) or population variance (false)
 * @return double 方差/Variance
 * 
 * @throws std::invalid_argument 如果样本方差且元素少于2/If sample variance and less than 2 elements
 * @throws std::invalid_argument 如果总体方差且容器为空/If population variance and container is empty
 * 
 * @code
 * std::vector<double> v{1.0, 2.0, 3.0};
 * double var = toolbox::math::variance(v); // 样本方差/sample variance
 * double var_pop = toolbox::math::variance(v, false); // 总体方差/population variance
 * @endcode
 */
template<typename TContainer>
double variance(const TContainer& data, bool sample_variance = true)
{
  ...
  if (sample_variance) {
    ...
    check_empty(data, func_name_str);  // 总体方差至少需要1个元素/Population variance requires at least 1 element
  }
...
/**
 * @brief 计算标准差（默认样本标准差）/Compute the standard deviation (default is sample standard deviation)
 * 
 * @tparam TContainer 容器类型，元素需为算术类型/Container type, elements must be arithmetic
 * @param data 输入容器/Input container
 * @param sample_stdev 是否计算样本标准差（true）或总体标准差（false）/Whether to compute sample stdev (true) or population stdev (false)
 * @return double 标准差/Standard deviation
 * 
 * @throws std::invalid_argument 如果样本标准差且元素少于2/If sample stdev and less than 2 elements
 * @throws std::invalid_argument 如果总体标准差且容器为空/If population stdev and container is empty
 * 
 * @code
 * std::vector<double> v{1.0, 2.0, 3.0};
 * double sd = toolbox::math::stdev(v); // 样本标准差/sample stdev
 * double sd_pop = toolbox::math::stdev(v, false); // 总体标准差/population stdev
 * @endcode
 */
template<typename TContainer>
double stdev(const TContainer& data, bool sample_stdev = true)
{
  ...
  // variance 函数会处理大小检查和空容器情况/variance will handle size and empty checks
  return std::sqrt(variance(data, sample_stdev));
}

/**
 * @brief 计算容器中元素的总和（返回容器元素类型）/Compute the sum of elements in the container (returns container element type)
 * 
 * @tparam TContainer 容器类型，元素需为算术类型/Container type, elements must be arithmetic
 * @param data 输入容器/Input container
...
typename TContainer::value_type sum(const TContainer& data)
{
  ...
  // 注意：对于较小整数类型和大量数据，这里可能溢出。考虑使用 sum_d。/Note: For small integer types and large data, may overflow. Consider using sum_d.
  return std::accumulate(data.begin(), data.end(), static_cast<ValueType>(0));
}

/**
 * @brief 计算容器中元素的总和（返回 double 类型以保证精度和范围）/Compute the sum of elements in the container (returns double for precision and range)
 * 
 * @tparam TContainer 容器类型，元素需为算术类型/Container type, elements must be arithmetic
 * @param data 输入容器/Input container
...
{
  using ValueType = typename TContainer::value_type;
  // std::max_element 默认使用 operator</std::max_element uses operator< by default
  static_assert(toolbox::traits::is_less_than_comparable_v<ValueType>,
...
/**
 * @brief 计算容器中元素的全距（最大值-最小值）/Compute the range (max - min) of elements in the container
 * 
 * @tparam TContainer 容器类型，元素需为算术类型且可比较/Container type, elements must be arithmetic and comparable
 * @param data 输入容器/Input container
...
typename TContainer::value_type range(const TContainer& data)
{
  ...
  check_empty(data, "range");  // min 和 max 内部也会检查/min and max will also check internally

  // C++17 中 std::minmax_element 已经可用/std::minmax_element is available in C++17
  auto [min_it, max_it] = std::minmax_element(data.begin(), data.end());
  return static_cast<ValueType>(*max_it - *min_it);
}

/**
 * @brief 计算百分位数（使用最近邻等级法和线性插值）/Compute the percentile (using nearest-rank and linear interpolation)
 * 
 * @tparam TContainer 容器类型，元素需为算术类型且可比较/Container type, elements must be arithmetic and comparable
 * @param data 输入容器/Input container
...
double percentile(const TContainer& data, double p)
{
  ...
  // 线性插值法 (R-7 in R, type 7 in NumPy, "Excel.Exclusive" in Excel)/Linear interpolation (R-7 in R, type 7 in NumPy, "Excel.Exclusive" in Excel)
...
/**
 * @brief 返回容器中k个最小的元素，升序排列/Return the k smallest elements in the container, sorted ascending
 * 
 * @tparam TContainer 容器类型，元素需可比较/Container type, elements must be comparable
 * @param data 输入容器/Input container
 ...
 * 
 * @note 若k为0返回空集合，若k>=容器大小则返回所有元素排序/Returns empty if k==0, returns all sorted if k>=container size
 * 
 * @code
                                                   ...
                                                   size_t k)
{
  ...
  check_empty(data, "min_k");  // 如果 k > 0, data 不应为空/If k > 0, data should not be empty

  std::vector<ValueType> result;
  if (k
      >= data.size()) {  // 如果 k 大于或等于容器大小，返回所有元素排序后的结果/If k >= container size, return all sorted
    result.assign(data.begin(), data.end());
    std::sort(result.begin(), result.end());
  } else {
    result.resize(k);  // 必须先调整大小/Must resize first
    std::partial_sort_copy(data.begin(),
                           data.end(),  // 从 data 复制并部分排序到 result/Copy from data and partial sort to result
                           result.begin(),
                           result.end());
  }
  return result;
...
/**
 * @brief 返回容器中k个最大的元素，降序排列/Return the k largest elements in the container, sorted descending
 * 
 * @tparam TContainer 容器类型，元素需可比较/Container type, elements must be comparable
 * @param data 输入容器/Input container
 ...
 * @throws std::invalid_argument 如果k>0且容器为空/If k>0 and container is empty
 * 
 * @note 若k为0返回空集合，若k>=容器大小则返回所有元素降序排序/Returns empty if k==0, returns all sorted descending if k>=container size
 * 
 * @code
 * std::vector<int> v{5, 1, 3, 2, 4};
...
{
  using ValueType = typename TContainer::value_type;
  // std::partial_sort_copy with std::greater 需要元素可比较/std::partial_sort_copy with std::greater requires comparable elements
  // std::greater<T> 使用 operator>，或者等价地，operator</std::greater<T> uses operator> or equivalently operator<
  ...
  if (k >= data.size()) {
    ...
    std::sort(result.begin(),
              result.end(),
              std::greater<ValueType>());  // 使用 std::greater 进行降序排序/Use std::greater for descending sort
  } else {
    result.resize(k);
    std::partial_sort_copy(
        ...
        result.begin(),
        result.end(),
        std::greater<ValueType>());  // 使用 std::greater 获取最大的k个/Use std::greater to get top k
  }
  return result;
}
