...
// specific)
    ...
    constexpr bool CPP_TOOLBOX_NATIVE_IS_LITTLE = (std::endian::native == std::endian::little);
    constexpr bool CPP_TOOLBOX_NATIVE_IS_BIG = (std::endian::native == std::endian::big);
    ...
    #define CPP_TOOLBOX_ENDIAN_NAMESPACE cpp_toolbox::detail
        ...
        // Windows (MSVC, MinGW) is almost always little-endian on supported architectures (x86, x64, ARM)
        ...
        // Check __BYTE_ORDER__ defensively, but default to little if check fails? Common case is LE.
        #if defined(__BYTE_ORDER__) && __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
             #warning "Detected Big Endian macOS? This is unusual. Verify target architecture."
        ...
        // Common Linux/GCC/Clang way using predefined macros
            ...
            constexpr bool CPP_TOOLBOX_NATIVE_IS_BIG = true;
        #else
             #error "Unknown machine endianness detected (__BYTE_ORDER__ defined but not LE/BE)."
    ...
    #else
        // --- Fallback/Unknown ---
        #warning "Could not reliably determine platform endianness. Assuming Little Endian. Define CPP_TOOLBOX_FORCE_ENDIANNESS if necessary."
        ...
        // #error "Cannot determine platform endianness. Please define CPP_TOOLBOX_FORCE_ENDIANNESS (little or big)."
