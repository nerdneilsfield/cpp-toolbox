...
public:
   ...
   * @param f
   * 任务函数（可以是函数指针、lambda表达式、std::function等可调用对象）。
    ...
    -> std::future<typename std::invoke_result_t<F, Args...>>
{
  ...
  {  // 使用 relaxed 因为后续 enqueue 有自己的同步
  ...
  tasks_.enqueue(
      [task /* Capture shared_ptr */]()
      {
        ...
        // Using task.get() can provide a somewhat unique identifier for the
        // task instance std::cout << "Worker lambda started for task " <<
        // task.get() << std::endl;
        try {
          // Execute the actual packaged_task via the shared pointer
          (*task)();
          // std::cout << "Worker lambda successfully invoked packaged_task " <<
          // task.get() << std::endl;
          ...
          // Potentially re-throw or handle, but packaged_task should set the
          // future state
        } catch (const std::exception& e) {
          std::cerr << "Worker lambda caught unexpected exception during "
                       "packaged_task invocation for task "
                    << task.get() << ": " << e.what() << std::endl;
          // If the packaged_task itself fails to set the exception on the
          // future, the future might remain unset, leading to errors later.
          // Setting the promise state here is complex and might interfere with
          // packaged_task. For now, just log.
        } catch (...) {
          std::cerr << "Worker lambda caught unknown exception during "
                       "packaged_task invocation for task "
                    << task.get() << std::endl;
        }
        // std::cout << "Worker lambda finished for task " << task.get() <<
        // std::endl;
      });
