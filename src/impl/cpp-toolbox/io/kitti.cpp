                          ...
                          std::unique_ptr<base_file_data_t>& data)
{
  ...
  // KITTI格式没有头部，直接读取二进制数据/KITTI format has no header, directly
  // read binary data
  ...
  if (success) {
    ...
    data = std::move(
        cloud);  // 转移所有权到输出参数/Transfer ownership to output parameter
    return true;
  }
...
bool kitti_format_t::write(const std::string& path,
                           ...
                           bool binary) const
{
  // KITTI格式总是二进制，忽略binary参数/KITTI format is always binary, ignore
  // binary parameter
  if (!data) {
          ...
          dynamic_cast<const point_cloud_t<float>*>(data.get()))
  {
    ...
    return write_internal(
        path, *cloud_f, true);  // KITTI总是二进制/KITTI is always binary
  }
          ...
          dynamic_cast<const point_cloud_t<double>*>(data.get()))
  {
    ...
    return write_internal(
        path, *cloud_d, true);  // KITTI总是二进制/KITTI is always binary
  }
...
// 显式实例化独立辅助函数/Explicit instantiations for standalone helper
// functions
...
template CPP_TOOLBOX_EXPORT bool write_kitti_bin<float>(
    const std::string& path, const toolbox::types::point_cloud_t<float>& cloud);
