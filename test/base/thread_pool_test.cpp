...
TEST_CASE("ThreadPool Construction and Destruction", "[base][thread_pool]") {
    ...
    SECTION("Destruction joins threads") {
         ...
         // Exact state depends on timing, but destructor should block until joinable.
     ...
     SECTION("Submitting to stopped pool (via future)") {
         ...
         // Relying on submit()'s internal check 'if(stop_)' is the intended safeguard.
         // We will test that check implicitly by ensuring submit works before destruction.

         // Example: Trying to submit after destruction would require unsafe practices
         // auto pool_ptr = std::make_unique<thread_pool_t>(1);
         // pool_ptr.reset(); // Destruct the pool
         // // Any operation on pool_ptr after this is UB.
         // REQUIRE_THROWS_AS(pool_ptr->submit([]{}), std::runtime_error); // Cannot do this

         // We'll skip actively testing submission after stop for now,
         // as it requires either a modified thread pool design (e.g., explicit stop method)
         // or unsafe test patterns.
         SUCCEED("Skipping active test for submission to stopped pool due to complexity/safety.");
     }
}
 ...
 */
TEST_CASE("ThreadPool Concurrent Submissions", "[base][thread_pool][multithreaded]") {
...
TEST_CASE("ThreadPool Move-Only Task Arguments", "[base][thread_pool]") {
    ...
    SECTION("Submit lambda capturing unique_ptr") {
        ...
        auto future = pool.submit([p = std::move(ptr)]() mutable {
            if (p) {
                int val = *p;
