 ...
 */
TEST_CASE("LockFreeQueue Single Thread Basic Operations", "[container][lock_free_queue]") {
...
/**
 * @brief Tests lock_free_queue_t with multiple producer threads and one consumer thread.
 */
TEST_CASE("LockFreeQueue Multi-Producer Single-Consumer (MPSC)", "[container][lock_free_queue][multithreaded]") {
    lock_free_queue_t<int> queue;
...
/**
 * @brief Tests lock_free_queue_t with one producer thread and multiple consumer threads.
 */
TEST_CASE("LockFreeQueue Single-Producer Multi-Consumer (SPMC)", "[container][lock_free_queue][multithreaded]") {
    lock_free_queue_t<int> queue;
    ...
    std::vector<std::vector<int>> consumer_results(num_consumers); // Per-consumer results
    ...
    auto consumer_func = [&](int consumer_id) {
        int value;
        while (consumed_count.load() < total_items) { // Check global consumed count
            if (queue.try_dequeue(value)) {
                consumer_results[consumer_id].push_back(value); // Store locally
                consumed_count++;
            } else {
                 // If producer might be finished and queue is empty, break
                 if (produced_count.load() == total_items && queue.try_dequeue(value) == false) {
                    // Double check after producer is done
                    if (!queue.try_dequeue(value)) break; // exit if still empty
                    else { // got one element, process it and continue
                         consumer_results[consumer_id].push_back(value);
                         consumed_count++;
                    }
                 } else {
                    std::this_thread::yield(); // Yield if queue might still receive items
                 }
            }
        }
         // Clean up retired nodes for this consumer thread
...
/**
 * @brief Tests lock_free_queue_t with multiple producer and multiple consumer threads (MPMC).
 */
TEST_CASE("LockFreeQueue Multi-Producer Multi-Consumer (MPMC)", "[container][lock_free_queue][multithreaded]") {
    lock_free_queue_t<int> queue;
    const int num_producers = 4;
    const int num_consumers = 4;
    const int items_per_producer = 1000;
    ...
    auto consumer_func = [&](int consumer_id) {
        ...
        while(queue.try_dequeue(v_final)) {
             ...
             consumed_count++; // Note: This might overshoot total_items slightly, but helps drain
        }
        lock_free_queue_t<int>::cleanup_this_thread_retired_nodes();
    };
    ...
    for (auto& c : consumers) {
     ...
     // Verification (final count might be slightly off due to race on consumed_count)
    std::set<int> consumed_set;
    int total_consumed_locally = 0;
    for(const auto& results : consumer_results) {
        total_consumed_locally += results.size();
    ...
    REQUIRE(total_consumed_locally == total_items); // Check exact count collected
    REQUIRE(consumed_set.size() == total_items);     // Check for duplicates/missing
