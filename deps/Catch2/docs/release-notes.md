...
* `catch_discover_tests` now supports skipped tests (#2873)
  * You can get the old behavior by calling `catch_discover_tests` with `SKIP_IS_FAILURE` option.
...
* Column wrapping ignores ansi color codes when calculating string width (#2833, #2849)
  * This makes the output much more readable when the provided messages contain color codes.
...
### Miscellaneous
* Catch2's build no longer leaks `-ffile-prefix-map` setting  to dependencies (#2533)
...
* Console color support has been rewritten and significantly improved
  * The color implementation based on ANSI color codes is always available
  * Color implementations respect their associated stream
    * previously e.g. Win32 impl would change console color even if Catch2 was writing to a file
  * The color API is resilient against changing evaluation order of expressions
...
* Fixed Windows console color handling for tests that redirect stdout (#2345)
...
* Running 0 tests (e.g. due to empty binary, or test spec not matching anything) returns non-0 exit code
  * Flag `--allow-running-no-tests` overrides this behavior.
...
* `ParseAndAddCatchTests` supports `CMP0110` policy for changing behavior of `add_test` (#2057)
...
* ANSI color code output now respects `-o` flag and writes to the file as well (#1502)
...
* MinGW is now detected as Windows platform w/o SHE support (#1257)
...
* Added `BrightYellow` color for use in reporters (#979)
...
### Fixes
* Don't use console color if running in XCode
...
* Compilation for Universal Windows Platform (UWP) is supported
  * SHE handling and colorized output are disabled when compiling for UWP
...
* TAP reporter now behaves as if `-s` was always set
  * This should be more consistent with the protocol desired behavior.
...
* Catch no longer attempts to reconstruct expression that led to a fatal error  (#810)
  * This fixes possible signal/SHE loop when processing expressions, where the signal was triggered by expression decomposition.
...
* Fixed SHE-related compilation error under older MinGW compilers, by making Windows SHE handling opt-in for compilers other than MSVC.
  * For specifics, look into the [documentation](configuration.md#top).
* Fixed compilation error under MinGW caused by improper compiler detection.
* Fixed XML reporter sometimes leaving an empty output file when a test ends with signal/structured exception.
* Fixed XML reporter not reporting captured stdout/stderr.
* Fixed possible infinite recursion in Windows SHE.
...
* Catch now supports SHE on Windows as well as signals on Linux.
