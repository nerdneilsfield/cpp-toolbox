...
[Terminal color](#terminal-color)<br>
...
## Terminal color
    ...
    CATCH_CONFIG_COLOUR_WIN32     // Force enables compiling coloring impl based on Win32 console API
    CATCH_CONFIG_NO_COLOUR_WIN32  // Force disables ...

Yes, Catch2 uses the british spelling of color.

Catch2 attempts to autodetect whether the Win32 console coloring API,
`SetConsoleTextAttribute`, is available, and if it is available it compiles
in a console coloring implementation that uses it.
...
1) If there is an error that would normally be signaled by an exception,
...
There is also a customization point for the exact behavior of what
