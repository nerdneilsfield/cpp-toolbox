...
[Catch::StringMaker specialization](#catchstringmaker-specialization)<br>
[Catch::is_range specialization](#catchis_range-specialization)<br>
...
However, as a convenience, Catch provides the `CATCH_REGISTER_ENUM` helper macro that will generate the `StringMaker` specialization for you with minimal code.
...
CATCH_REGISTER_ENUM( Bikeshed::Colors,
    Bikeshed::Colors::Red,
    Bikeshed::Colors::Green,
    Bikeshed::Colors::Blue )

TEST_CASE() {
    REQUIRE( Bikeshed::Colors::Red == Bikeshed::Colors::Blue );
}
```
