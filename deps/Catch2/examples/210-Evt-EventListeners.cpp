...
namespace {
std::string was(int const level) {
...
void print( std::ostream& os, int const level, std::string const& title, Catch::SourceLineInfo const& info ) {
    os << was(level  ) << title << ":\n"
       << was(level+1) << "- file: " << info.file << "\n"
       << was(level+1) << "- line: " << info.line << "\n";
}
...
void print( std::ostream& os, int const level, Catch::MessageInfo const& info ) {
    os << was(level+1) << "- macroName: '" << info.macroName << "'\n"
       << was(level+1) << "- message '"    << info.message   << "'\n";
    print( os,level+1  , "- lineInfo", info.lineInfo );
    os << was(level+1) << "- sequence "    << info.sequence  << "\n";
}
...
void print( std::ostream& os, int const level, std::string const& title, Catch::Counts const& info ) {
    os << was(level  ) << title << ":\n"
       << was(level+1) << "- total(): "     << info.total()     << "\n"
       << was(level+1) << "- allPassed(): " << info.allPassed() << "\n"
       << was(level+1) << "- allOk(): "     << info.allOk()     << "\n"
       << was(level+1) << "- passed: "      << info.passed      << "\n"
       << was(level+1) << "- failed: "      << info.failed      << "\n"
       << was(level+1) << "- failedButOk: " << info.failedButOk << "\n";
}
...
void print( std::ostream& os, int const level, std::string const& title, Catch::TestRunStats const& info ) {
    ...
    os << was(level+1) << "- aborting: " << info.aborting << "\n";
}
...
void print( std::ostream& os, int const level, std::string const& title, Catch::TestCaseInfo const& info ) {
    os << was(level  ) << title << ":\n"
       << was(level+1) << "- isHidden(): "       << info.isHidden() << "\n"
       << was(level+1) << "- throws(): "         << info.throws() << "\n"
       << was(level+1) << "- okToFail(): "       << info.okToFail() << "\n"
       << was(level+1) << "- expectedToFail(): " << info.expectedToFail() << "\n"
       << was(level+1) << "- tagsAsString(): '"  << info.tagsAsString() << "'\n"
       << was(level+1) << "- name: '"            << info.name << "'\n"
       << was(level+1) << "- className: '"       << info.className << "'\n"
       << was(level+1) << "- tags: "             << info.tags << "\n";
    print( os, level+1 , "- lineInfo", info.lineInfo );
    os << was(level+1) << "- properties (flags): 0x" << std::hex << static_cast<uint32_t>(info.properties) << std::dec << "\n";
}
...
void print( std::ostream& os, int const level, std::string const& title, Catch::TestCaseStats const& info ) {
    ...
    print( os, level+1 , "- totals"  , info.totals   );
    os << was(level+1) << "- stdOut: "   << info.stdOut << "\n"
       << was(level+1) << "- stdErr: "   << info.stdErr << "\n"
       << was(level+1) << "- aborting: " << info.aborting << "\n";
}
...
void print( std::ostream& os, int const level, std::string const& title, Catch::SectionStats const& info ) {
    ...
    print( os, level+1 , "- assertions" , info.assertions );
    os << was(level+1) << "- durationInSeconds: " << info.durationInSeconds << "\n"
       << was(level+1) << "- missingAssertions: " << info.missingAssertions << "\n";
}
...
void print( std::ostream& os, int const level, std::string const& title, Catch::AssertionInfo const& info ) {
    os << was(level  ) << title << ":\n"
       << was(level+1) << "- macroName: '"  << info.macroName << "'\n";
    print( os, level+1 , "- lineInfo" , info.lineInfo );
    os << was(level+1) << "- capturedExpression: '" << info.capturedExpression << "'\n"
       << was(level+1) << "- resultDisposition (flags): 0x" << std::hex << info.resultDisposition  << std::dec << "\n";
}
...
void print( std::ostream& os, int const level, std::string const& title, Catch::AssertionResultData const& info ) {
    os << was(level  ) << title << ":\n"
       << was(level+1) << "- reconstructExpression(): '" <<   info.reconstructExpression() << "'\n"
       << was(level+1) << "- message: '"                 <<   info.message << "'\n"
       << was(level+1) << "- lazyExpression: '"          << "(info.lazyExpression)" << "'\n"
       << was(level+1) << "- resultType: '"              <<   info.resultType << "'\n";
}
...
void print( std::ostream& os, int const level, std::string const& title, Catch::AssertionResult const& info ) {
    os << was(level  ) << title << ":\n"
       << was(level+1) << "- isOk(): "  << info.isOk() << "\n"
       << was(level+1) << "- succeeded(): "  << info.succeeded() << "\n"
       << was(level+1) << "- getResultType(): "  << info.getResultType() << "\n"
       << was(level+1) << "- hasExpression(): "  << info.hasExpression() << "\n"
       << was(level+1) << "- hasMessage(): "  << info.hasMessage() << "\n"
       << was(level+1) << "- getExpression(): '"  << info.getExpression() << "'\n"
       << was(level+1) << "- getExpressionInMacro(): '"  << info.getExpressionInMacro()  << "'\n"
       << was(level+1) << "- hasExpandedExpression(): "  << info.hasExpandedExpression() << "\n"
       << was(level+1) << "- getExpandedExpression(): "  << info.getExpandedExpression() << "'\n"
       << was(level+1) << "- getMessage(): '"  << info.getMessage() << "'\n";
    print( os, level+1 , "- getSourceInfo(): ", info.getSourceInfo() );
    os << was(level+1) << "- getTestMacroName(): '"  << info.getTestMacroName() << "'\n";
