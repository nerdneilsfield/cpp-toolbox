...
TEST_CASE("analyze", "[approvals][benchmark]") {
    ...
    auto analysis = Catch::Benchmark::Detail::analyze(config, samples.data(), samples.data() + samples.size());
...
TEST_CASE("analyze no analysis", "[benchmark]") {
    Catch::ConfigData data{};
    data.benchmarkConfidenceInterval = 0.95;
