...
TEST_CASE( "Process can be configured on command line", "[config][command-line]" ) {
    ...
    SECTION( "use-colour") {
        ...
        SECTION( "error" ) {
            ...
            CHECK( !result );
            CHECK_THAT( result.errorMessage(), ContainsSubstring( "color mode must be one of" ) );
        }
    }
...
TEST_CASE("Win32 color implementation is compile-time optional",
          "[approvals][cli][colors]") {
    Catch::ConfigData config;
    auto cli = Catch::makeCommandLineParser( config );
