...
namespace {
    class TestColourImpl : public Catch::ColourImpl {
        ...
        void use( Catch::Color::Code colourCode ) const override {
...
TEST_CASE("ColourGuard behavior", "[console-colours]") {
    ...
    SECTION("ColourGuard is disengaged by default") {
        { auto guard = colourImpl.guardColour( Catch::Color::Red ); }

        REQUIRE( streamWrapper.str().empty() );
    }

    SECTION("ColourGuard is engaged by op<<") {
        stream << "1\n" << colourImpl.guardColour( Catch::Color::Red ) << "2\n";
        stream << "3\n";
    ...
    SECTION("ColourGuard can be engaged explicitly") {
        {
            auto guard =
                colourImpl.guardColour( Catch::Color::Red ).engage( stream );
            stream << "A\n"
                   << "B\n";
        }
        stream << "C\n";
