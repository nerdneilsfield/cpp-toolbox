...
TEST_CASE("Validate SHE behavior - handled", "[approvals][FatalConditionHandler][CATCH_PLATFORM_WINDOWS]")
{
    // Validate that Catch2 framework correctly handles tests raising and handling SHE exceptions.
    throw_and_catch();
}
...
TEST_CASE("Validate SHE behavior - unhandled", "[.approvals][FatalConditionHandler][CATCH_PLATFORM_WINDOWS]")
{
    // Validate that Catch2 framework correctly handles tests raising and not handling SHE exceptions.
    throw_no_catch();
}
...
TEST_CASE("Validate SHE behavior - no crash for stack unwinding", "[approvals][!throws][!shouldfail][FatalConditionHandler][CATCH_PLATFORM_WINDOWS]")
{
    // Trigger stack unwinding with SHE top-level filter changed and validate the test fails expectedly with no application crash
