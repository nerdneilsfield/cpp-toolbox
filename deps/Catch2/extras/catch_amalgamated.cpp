...
namespace Catch {
    namespace Benchmark {
        namespace Detail {
            SampleAnalysis analyze(const IConfig &cfg, FDuration* first, FDuration* last) {
...
namespace Catch {
    ...
    Session::Session() {
        ...
        if ( !exceptions.empty() ) {
            ...
            auto guard = colourImpl->guardColour( Color::Red );
    ...
    int Session::applyCommandLine( int argc, char const * const * argv ) {
        ...
        if( !result ) {
            ...
            getCurrentMutableContext().setConfig(m_config.get());
            auto errStream = makeStream( "%stderr" );
            auto color = makeColourImpl( ColourMode::PlatformDefault, errStream.get() );

            errStream->stream()
                << colour->guardColour( Color::Red )
                << "\nError(s) in input:\n"
            ...
            ParserResult convertInto( std::string const& source,
                ...
                if ( srcLC == "y" || srcLC == "1" || srcLC == "true" ||
                    ...
                    return ParserResult::runtimeError(
                        "Expected a boolean value but did not recognize: '" +
                        source + '\'' );
                }
                return ParserResult::ok( ParseResultType::Matched );
            }
...
namespace Catch {
    ...
    Clara::Parser makeCommandLineParser( ConfigData& config ) {
        ...
        auto const setWarning = [&]( std::string const& warning ) {
            ...
            return ParserResult ::runtimeError(
                "Unrecognized warning option: '" + warning + '\'' );
        };
        ...
        auto const setTestOrder = [&]( std::string const& order ) {
                if( startsWith( "declared", order ) )
                    ...
                    return ParserResult::runtimeError( "Unrecognized ordering: '" + order + '\'' );
                return ParserResult::ok( ParseResultType::Matched );
            };
        ...
        auto const setDefaultColourMode = [&]( std::string const& colourMode ) {
            ...
            if ( !maybeMode ) {
                return ParserResult::runtimeError(
                    "color mode must be one of: default, ansi, win32, "
                    "or none. '" +
                    colourMode + "' is not recognized" );
            }
            auto mode = *maybeMode;
            if ( !isColourImplAvailable( mode ) ) {
                return ParserResult::runtimeError(
                    "color mode '" + colourMode +
...
namespace Catch {
    ...
    namespace Detail {
        ...
        namespace {
            ...
            // In other words, it returns the Blue part of Bikeshed::Color::Blue
...
namespace Catch {
    ...
    // If neither SHE nor signal handling is required, the handler impls
...
#if defined( CATCH_CONFIG_WINDOWS_SEH ) && defined( CATCH_CONFIG_POSIX_SIGNALS )
#error "Inconsistent configuration: Windows' SHE handling and POSIX signals cannot be enabled at the same time"
#endif // CATCH_CONFIG_WINDOWS_SEH && CATCH_CONFIG_POSIX_SIGNALS
...
namespace Catch {
    ...
    void FatalConditionHandler::disengage_platform() noexcept {
        if (SetUnhandledExceptionFilter(previousTopLevelExceptionFilter) != topLevelExceptionFilter) {
            Catch::cerr()
                << "Unexpected SHE unhandled exception filter on disengage."
...
namespace {
    ...
    // Color::LightGrey
    static constexpr Color::Code compactDimColour = Color::FileName;
...
public:
    ...
    void print() {
        ...
        case ResultWas::Ok:
            printResultType(Color::ResultSuccess, compactPassedString);
        ...
        case ResultWas::ExpressionFailed:
            if (result.isOk())
                printResultType(Color::ResultSuccess, compactFailedString + " - but was ok"_sr);
            else
                printResultType(Color::Error, compactFailedString);
            printOriginalExpression();
            printReconstructedExpression();
            printRemainingMessages();
            break;
        ...
        case ResultWas::DidntThrowException:
            ...
            break;
        case ResultWas::Info:
            printResultType(Color::None, "info"_sr);
            printMessage();
            printRemainingMessages();
            break;
        case ResultWas::Warning:
            printResultType(Color::None, "warning"_sr);
            printMessage();
            printRemainingMessages();
            break;
        ...
        case ResultWas::ExplicitSkip:
            printResultType(Color::Skip, "skipped"_sr);
            printMessage();
            printRemainingMessages();
            break;
...
private:
    void printSourceInfo() const {
        stream << colourImpl->guardColour( Color::FileName )
               << result.getSourceInfo() << ':';
    }

    void printResultType(Color::Code color, StringRef passOrFail) const {
        if (!passOrFail.empty()) {
            stream << colourImpl->guardColour(color) << ' ' << passOrFail;
            stream << ':';
        }
    }
    ...
    void printRemainingMessages(Color::Code color = compactDimColour) {
        if (itMessage == messages.end())
            return;
        ...
        stream << colourImpl->guardColour( color ) << " with "
    ...
    ColourImpl* colourImpl;
        ...
        void CompactReporter::testRunStarting( TestRunInfo const& ) {
            if ( m_config->testSpec().hasFilters() ) {
                m_stream << m_colour->guardColour( Color::BrightYellow )
                         << "Filters: "
                         << m_config->testSpec()
                         << '\n';
            }
        ...
        void CompactReporter::sectionEnded(SectionStats const& _sectionStats) {
            double due = _sectionStats.durationInSeconds;
            if ( shouldShowDuration( *m_config, due ) ) {
                m_stream << getFormattedDuration( due ) << " s: " << _sectionStats.sectionInfo.name << '\n' << std::flush;
            }
        }
...
private:
    ...
    void printOriginalExpression() const {
        if (result.hasExpression()) {
            stream << colourImpl->guardColour( Color::OriginalExpression )
    ...
    void printReconstructedExpression() const {
        if (result.hasExpandedExpression()) {
            ...
            stream << colourImpl->guardColour( Color::ReconstructedExpression )
...
void ConsoleReporter::sectionEnded(SectionStats const& _sectionStats) {
    ...
    if (_sectionStats.missingAssertions) {
        lazyPrint();
        auto guard =
            m_colour->guardColour( Color::ResultError ).engage( m_stream );
...
void ConsoleReporter::benchmarkFailed( StringRef error ) {
    auto guard = m_colour->guardColour( Color::Red ).engage( m_stream );
    (*m_tablePrinter)
...
void ConsoleReporter::testRunStarting(TestRunInfo const& _testRunInfo) {
    ...
    if ( m_config->testSpec().hasFilters() ) {
        m_stream << m_colour->guardColour( Color::BrightYellow ) << "Filters: "
                 << m_config->testSpec() << '\n';
    }
...
void ConsoleReporter::lazyPrintRunInfo() {
    m_stream << '\n'
             << lineOfChars( '~' ) << '\n'
             << m_colour->guardColour( Color::SecondaryText )
...
void ConsoleReporter::printTestCaseAndSectionHeader() {
    ...
    if (m_sectionStack.size() > 1) {
        auto guard = m_colour->guardColour( Color::Headers ).engage( m_stream );
    ...
    m_stream << lineOfChars( '-' ) << '\n'
             << m_colour->guardColour( Color::FileName ) << lineInfo << '\n'
             << lineOfChars( '.' ) << "\n\n"
             << std::flush;
}
...
void ConsoleReporter::printTotalsDivider(Totals const& totals) {
    if (totals.testCases.total() > 0) {
        ...
        m_stream << m_colour->guardColour( Color::Error )
                 << std::string( failedRatio, '=' )
                 << m_colour->guardColour( Color::ResultExpectedFailure )
                 << std::string( failedButOkRatio, '=' );
        if ( totals.testCases.allPassed() ) {
            m_stream << m_colour->guardColour( Color::ResultSuccess )
                     << std::string( passedRatio, '=' );
        } else {
            m_stream << m_colour->guardColour( Color::Success )
                     << std::string( passedRatio, '=' );
        }
        m_stream << m_colour->guardColour( Color::Skip )
                 << std::string( skippedRatio, '=' );
    } else {
        m_stream << m_colour->guardColour( Color::Warning )
...
namespace Catch {
    ...
    void defaultListTests(std::ostream& out, ColourImpl* streamColour, std::vector<TestCaseHandle> const& tests, bool isFiltered, Verbosity verbosity) {
        ...
        for (auto const& test : tests) {
            ...
            Color::Code color = testCaseInfo.isHidden()
                ? Color::SecondaryText
                : Color::None;
            auto colourGuard = streamColour->guardColour( color ).engage( out );
    ...
    namespace {
        class SummaryColumn {
        public:
            SummaryColumn( std::string suffix, Color::Code color ):
                m_suffix( CATCH_MOVE( suffix ) ), m_colour( color ) {}
            ...
            Color::Code getColour() const { return m_colour; }
            ...
            for ( auto const& col : cols ) {
                ...
                if ( suffix.empty() ) {
                    ...
                    if ( value != "0" ) {
                        ...
                        stream << color.guardColour( Color::Warning )
                               << "- none -";
                    }
                } else if ( value != "0" ) {
                    stream << color.guardColour( Color::LightGrey ) << " | "
                           << color.guardColour( col.getColour() ) << value
                           << ' ' << suffix;
                }
            }
            stream << '\n';
    ...
    void printTestRunTotals( std::ostream& stream,
                             ...
                             Totals const& totals ) {
        if ( totals.testCases.total() == 0 ) {
            stream << streamColour.guardColour( Color::Warning )
                   << "No tests ran\n";
            return;
        }

        if ( totals.assertions.total() > 0 && totals.testCases.allPassed() ) {
            stream << streamColour.guardColour( Color::ResultSuccess )
                   << "All tests passed";
            stream << " ("
        ...
        columns.push_back( SummaryColumn( "skipped", Color::Skip )
                               .addRow( totals.testCases.skipped )
                               // Don't print "skipped assertions"
                               .addRow( 0 ) );
        columns.push_back(
            SummaryColumn( "failed as expected", Color::ResultExpectedFailure )
...
namespace Catch {
    ...
    namespace {
        ...
        static constexpr Color::Code tapDimColour = Color::FileName;
        ...
        private:
            ...
            void printRemainingMessages(Color::Code color = tapDimColour) {
                if (itMessage == messages.end()) {
                    return;
                }
