...
namespace Catch {
    ...
    enum class ColourMode : std::uint8_t {
        ...
        PlatformDefault,
        //! Use ANSI color code escapes
        ANSI,
        //! Use Win32 console color API
        Win32,
        //! Don't use any color
        None
    };
        ...
        struct Benchmark {
            ...
            void run() {
                ...
                CATCH_TRY{
                    ...
                    auto analysis = Detail::analyze(*cfg, samples.data(), samples.data() + samples.size());
...
namespace Catch {
     ...
     * that the color mode is known to Catch2, but it might not be
...
namespace Catch {
    ...
    namespace Generators {
        ...
        public:
             ...
             * features.
             *
             * As with `next`, returns true if the move succeeded and
...
#undef Digits
        ...
        constexpr ExtendedMultResult<unit> extendedMult( unit lhs, unit rhs ) {
            static_assert( std::is_unsigned<unit>::value,
                           "extendedMult can only handle unsigned integers" );
            static_assert( sizeof( unit ) < sizeof( std::uint64_t ),
                           "Generic extendedMult can only handle types smaller "
                           "than uint64_t" );
            using WideType = DoubleWidthUnsignedType_t<unit>;

            auto result = WideType( lhs ) * WideType( rhs );
            return {
                static_cast<unit>( result >> ( CHAR_BIT * sizeof( unit ) ) ),
                static_cast<unit>( result & unit( -1 ) ) };
        }
...
class uniform_floating_point_distribution {
    ...
    static_assert(!std::is_same<FloatType, long double>::value,
                  "We do not support long double due to inconsistent behavior between platforms");
...
namespace Catch {
    ...
    //! By setting up its preferences, a reporter can modify Catch2's behavior
...
namespace Catch {
    ...
    /**
     * Wrapper for platform-specific fatal error (signals/SHE) handlers
     *
...
namespace Catch {
    ...
    public:
         ...
         *
         * This is useful for e.g. Win32 color support, because the Win32
...
namespace TestCaseTracking {
    ...
    public:
        ...
        bool isOpen() const;
        //! Returns true if tracker has started
...
namespace Catch {
    ...
    protected:
        ...
        //! Color implementation this reporter was configured for
