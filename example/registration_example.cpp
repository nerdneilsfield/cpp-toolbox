 ...
 * This example demonstrates how to use three coarse registration algorithms in cpp-toolbox:
 * 1. RANSAC - 基于对应关系的配准 / Correspondence-based registration
 * 2. 4PCS - 不需要初始对应关系的配准 / Registration without initial correspondences
 * 3. Super4PCS - 4PCS的优化版本，适合大规模点云 / Optimized version of 4PCS for large point clouds
 */
...
template<typename DataType>
std::shared_ptr<point_cloud_t<DataType>> load_pcd_file(const std::string& filename)
{
  auto cloud_unique = read_pcd<DataType>(filename);
  if (!cloud_unique) {
    std::cerr << "无法读取文件 / Failed to read file: " << filename << std::endl;
    return nullptr;
  }
...
// 示例1：RANSAC配准（需要特征匹配） / Example 1: RANSAC registration (requires feature matching)
void ransac_registration_example(const std::string& source_file, const std::string& target_file)
{
  ...
  auto source_cloud = load_pcd_file<DataType>(source_file);
  auto target_cloud = load_pcd_file<DataType>(target_file);
  
  if (!source_cloud || !target_cloud) {
    std::cerr << "错误：无法加载点云文件 / Error: Failed to load point cloud files\n";
    return;
  }
  
  std::cout << "源点云大小 / Source cloud size: " << source_cloud->size() << " 点/points\n";
  std::cout << "目标点云大小 / Target cloud size: " << target_cloud->size() << " 点/points\n";
  ...
  std::cout << "源关键点 / Source keypoints: " << src_keypoint_indices.size() << std::endl;
  std::cout << "目标关键点 / Target keypoints: " << tgt_keypoint_indices.size() << std::endl;
  
  if (src_keypoint_indices.size() < 10 || tgt_keypoint_indices.size() < 10) {
    std::cerr << "关键点太少，跳过RANSAC / Too few keypoints, skipping RANSAC\n";
    return;
  }
  ...
  std::cout << "计算了 " << src_descriptors->size() << " 个源描述子 / Computed source descriptors\n";
  std::cout << "计算了 " << tgt_descriptors->size() << " 个目标描述子 / Computed target descriptors\n";
  
  // 4. 生成对应关系 / Generate correspondences
  auto src_indices_ptr = std::make_shared<std::vector<std::size_t>>(src_keypoint_indices);
  auto tgt_indices_ptr = std::make_shared<std::vector<std::size_t>>(tgt_keypoint_indices);
  
  auto correspondences = generate_correspondences_brute_force<DataType, fpfh_signature_t<DataType>>(
      source_cloud, src_descriptors, src_indices_ptr,
      target_cloud, tgt_descriptors, tgt_indices_ptr,
      0.8f, true, false);
  
  std::cout << "生成了 " << correspondences.size() << " 个对应关系 / Generated correspondences\n";
  
  if (correspondences.size() < 3) {
    std::cerr << "对应关系太少，无法进行RANSAC / Too few correspondences for RANSAC\n";
    return;
  }
  ...
  if (ransac.align(result)) {
    ...
    std::cout << "  内点数 / Inliers: " << result.inliers.size() << "/" << correspondences.size() << "\n";
...
// 示例2：4PCS配准（不需要初始对应关系） / Example 2: 4PCS registration (no initial correspondences)
void four_pcs_registration_example(const std::string& source_file, const std::string& target_file)
{
  using DataType = float;
...
// 示例3：Super4PCS配准（智能索引优化） / Example 3: Super4PCS registration (smart indexing)
void super_four_pcs_registration_example(const std::string& source_file, const std::string& target_file)
{
  using DataType = float;
  
  std::cout << "\n========== Super4PCS 配准示例 / Super4PCS Registration Example ==========\n";
  ...
  super4pcs.enable_smart_indexing(true);    // 启用智能索引 / Enable smart indexing
...
void quick_registration_example(const std::string& source_file, const std::string& target_file)
{
  using DataType = float;
  ...
  auto result = quick_registration(source_cloud, target_cloud, "super4pcs", 0.5f);
...
{
  if (argc < 3) {
    std::cout << "用法 / Usage: " << argv[0] << " <source.pcd> <target.pcd> [algorithm]\n";
    std::cout << "  algorithm: ransac, 4pcs, super4pcs, quick, all (默认/default: all)\n";
    return 1;
  }
  ...
  if (algorithm == "quick" || algorithm == "all") {
    try {
      ...
      std::cerr << "快速配准错误 / Quick registration error: " << e.what() << std::endl;
    }
  }
